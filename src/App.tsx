...
const results: Record<ResultCode, ResultPack> = {
  SAE: {
    ...
    behaviors: { ko: ["집사가 움직이면 슬쩍 따라온다.", "같은 공간에 있으면 더 편해 보인다.", "무릎/옆자리 선호도가 높다."], th: ["ทาสขยับ…ฉันก็ขยับตาม", "อยู่ห้องเดียวกันแล้วดูสบายใจ", "ชอบยึดข้างๆ/ตักเป็นพิเศษ"] },
    heart: { ko: ["네가 움직이면 나도 따라가고,", "같은 공간에 있는 게 제일 편해.", "그게 내 방식의 애정이야."], th: ["เธอขยับ…ฉันก็ขยับตาม", "แค่อยู่ห้องเดียวกันก็อุ่นใจ", "นี่แหละวิธีบอกรักของฉัน"] },
    guide: { ko: ["하루에 5~10분이라도 매일 놀아줘.", "눈을 자주 마주치고 이름을 불러줘.", "갑자기 사라지지 말고, 존재감만 있어줘."], th: ["เล่นด้วยกันทุกวัน 5–10 นาทีพอ", "สบตา/เรียกชื่อบ่อยๆ ฉันชอบ", "อย่าหายเงียบๆ ฉันจะงงว่าไปไหน"] },
    caution: { ko: ["갑작스런 생활 루틴 변화는 불안할 수 있어.", "관심이 부족하면 집사 호출이 잦아질 수 있어."], th: ["เปลี่ยนรูทีนกะทันหัน ฉันจะไม่มั่นใจ", "ถ้าถูกละเลย…ฉันจะเรียกทาสบ่อยขึ้นเอง"] },
  },
  SAT: {
    ...
    behaviors: { ko: ["장난감만 꺼내면 눈빛이 달라진다.", "흥분하면 갑자기 뛰기 시작한다.", "큰 소음/갑작스런 자극엔 예민해질 수 있다."], th: ["เห็นของเล่นแล้วตาเปลี่ยนทันที", "คึกขึ้นมาแล้ววิ่งแบบไม่มีเหตุผล", "เสียงดัง/สิ่งกระตุ้นฉับพลันทำให้ไวได้"] },
    heart: { ko: ["에너지는 넘치는데,", "마음은 생각보다 섬세해.", "재밌으면 최고, 싫으면 바로 티 나."], th: ["พลังมีเต็มหลอด", "แต่ใจฉันบอบบางกว่าที่คิด", "สนุกก็สุด—ไม่ชอบก็ออกอาการทันที"] },
    guide: { ko: ["짧게 자주(3~5분×2~3회) 놀아줘.", "과열되면 잠깐 쉬는 시간(숨을 곳) 줘.", "소리/움직임 자극은 천천히 적응시키기."], th: ["เล่นสั้นๆ แต่บ่อย (3–5 นาที × 2–3 รอบ)", "คึกเกินไปให้พัก/มีที่หลบ", "เสียงดัง/การเคลื่อนไหวแรง ค่อยๆ ปรับ"] },
    caution: { ko: ["흥분이 쌓이면 공격/과잉행동으로 보일 수 있어.", "갑작스런 큰 소음은 스트레스가 될 수 있어."], th: ["คึกสะสมอาจดูเหมือนดุ/งับเล่นแรง", "เสียงดังฉับพลันทำให้เครียดได้"] },
  },
  SCE: {
    ...
    behaviors: { ko: ["밥/놀이 시간이 어긋나면 표정이 달라진다.", "정해진 자리(캣타워/창가)를 고집한다.", "다가오면 조용히 곁에 머문다."], th: ["เวลาข้าว/เล่นเลื่อนแล้วมีงอน", "ยึดมุมประจำ (คอนโด/หน้าต่าง)", "พอไว้ใจแล้วจะอยู่ใกล้ๆ แบบเงียบๆ"] },
    heart: { ko: ["익숙한 게 편하고,", "내 공간이 지켜지면 마음이 안정돼.", "조용히 곁에 있는 타입이야."], th: ["ฉันชอบความคุ้นเคย", "พื้นที่ของฉันชัดๆ แล้วใจสงบ", "ไม่ต้องหวือหวา แค่อยู่ใกล้ๆ ก็พอ"] },
    guide: { ko: ["밥/놀이/휴식 시간을 일정하게 유지해줘.", "정해진 자리(숨는 곳/관찰 자리)를 만들어줘.", "다가갈 땐 천천히, 예고하고 움직여줘."], th: ["เวลาข้าว/เล่น/พัก ให้สม่ำเสมอ", "มีมุมประจำ (ที่หลบ/จุดส่อง)", "เข้าหาแบบช้าๆ อย่าพุ่งใส่"] },
  ...
  SCT: {
    ...
    behaviors: { ko: ["새 사료/간식은 바로 안 먹고 검증부터 한다.", "새 물건은 멀리서 관찰 후 슬쩍 접근한다.", "마음 열리면 오래 가는 편이다."], th: ["ของกินใหม่ต้องตรวจสอบก่อน", "ของใหม่ขอส่องไกลๆ แล้วค่อยแตะ", "แต่ถ้าไว้ใจแล้วจะยาวๆ เลย"] },
    heart: { ko: ["낯선 건 경계하고,", "익숙한 게 제일 편해.", "마음이 열리면 오래 가는 편이야."], th: ["ของใหม่ฉันระแวงนิดนึง", "ของเดิมๆ คือสบายใจ", "แต่ถ้าไว้ใจแล้ว ฉันยาวๆ เลย"] },
    guide: { ko: ["사료/간식 바꿀 땐 7~10일에 걸쳐 천천히.", "새 물건은 한 번에 많이 두지 말고 하나씩.", "싫어하는 신호(피하기/숨기)를 존중해줘."], th: ["เปลี่ยนอาหาร/ขนม ค่อยๆ 7–10 วัน", "ของใหม่ทีละชิ้น อย่าถาโถม", "ถ้าฉันหลบ/ไม่เอา ให้เคารพสัญญาณ"] },
  ...
  IAE: {
    ...
    behaviors: { ko: ["밤에 갑자기 질주 타임이 열린다.", "낮엔 얌전하다가 밤에 각성한다.", "혼자 놀 장난감이 있으면 만족도가 오른다."], th: ["กลางคืนอยู่ดีๆ ก็วิ่งเหมือนมีงานแข่ง", "กลางวันชิล—กลางคืนตื่น", "มีของเล่นเล่นเองแล้วแฮปปี้ขึ้น"] },
    heart: { ko: ["조용해지면 에너지가 차오르고,", "그때가 제일 자유로워.", "그래서 갑자기 뛰기 시작해."], th: ["พอบ้านเงียบ…พลังฉันก็มาจ้า", "ตอนนั้นแหละอิสระสุดๆ", "เลยต้องวิ่งให้โลกรู้"] },
    guide: { ko: ["잠들기 전 10분 ‘마지막 놀이’ 추천.", "밤에 혼자 놀 장난감(공/트랙) 준비.", "낮에 활동을 분산(창가 놀이/간식 퍼즐)."], th: ["ก่อนนอนเล่น 10 นาที ‘รอบสุดท้าย’", "เตรียมของเล่นเล่นคนเดียวตอนกลางคืน", "กระจายกิจกรรมตอนกลางวัน (หน้าต่าง/ของเล่นปริศนา)"] },
  ...
  IAT: {
    ...
    behaviors: { ko: ["부르면 안 오다가, 자기가 원할 때만 온다.", "스킨십은 ‘내 타이밍’이 중요하다.", "혼자 놀거리만 있어도 잘 지낸다."], th: ["เรียกไม่มา…แต่เดี๋ยวฉันอยากมาก็มาเอง", "กอดได้เมื่อฉันอนุญาต", "มีของเล่นเล่นเองแล้วอยู่ได้สบาย"] },
    heart: { ko: ["혼자 있는 시간이 편하고,", "내가 정한 타이밍에 다가가고 싶어.", "그래도 신뢰는 천천히 쌓여."], th: ["อยู่คนเดียวสบายใจ", "จะเข้าใกล้เมื่อฉันพร้อม", "ไว้ใจได้…แต่ขอเวลาหน่อย"] },
    guide: { ko: ["혼자 놀 거리(터널/스크래처/트랙)를 만들어줘.", "스킨십은 ‘선택권’을 줘 (다가오면 OK).", "사냥놀이(낚싯대/숨바꼭질)로 교감."], th: ["มีของเล่นเล่นคนเดียว (อุโมงค์/ที่ลับเล็บ/รางบอล)", "ให้ ‘สิทธิ์เลือก’ เรื่องกอด ถ้าฉันมาเองคือโอเค", "เล่นแนวล่า (ไม้ตกแมว/ซ่อนหา) เพื่อเชื่อมใจ"] },
  ...
  ICE: {
    ...
    behaviors: { ko: ["가까이 붙으면 피하고, 조금 떨어지면 곁에 온다.", "같은 공간에서 ‘동거’하는 걸 좋아한다.", "먼저 다가올 때까지 기다리면 관계가 좋아진다."], th: ["ติดมากไปฉันหลบ…แต่ถ้าพอดีๆ ฉันจะมาเอง", "อยู่ห้องเดียวกันแบบไม่วุ่นวายคือดีที่สุด", "รอให้ฉันเข้าหาเองแล้วจะเวิร์ก"] },
    heart: { ko: ["혼자가 편한데,", "네가 너무 멀어지는 건 또 싫어.", "적당한 거리가 딱 좋아."], th: ["ฉันชอบอยู่เงียบๆ", "แต่ถ้าเธอหายไปก็ไม่โอเค", "ระยะพอดีๆ คือดีที่สุด"] },
    guide: { ko: ["내가 먼저 다가올 때까지 기다려줘.", "같은 공간에 조용히 있어주는 게 최고.", "숨을 공간(박스/캣타워 아래) 확보해줘."], th: ["รอให้ฉันเข้าหาเอง", "อยู่ห้องเดียวกันแบบเงียบๆ คือรักแล้ว", "เตรียมที่หลบ (กล่อง/ใต้คอนโดแมว)"] },
  ...
  ICT: {
    ...
    behaviors: { ko: ["손님/새 물건은 멀리서 관찰부터 한다.", "갑작스런 변화가 반복되면 예민해진다.", "안전하다고 느끼면 천천히 거리를 좁힌다."], th: ["คนใหม่/ของใหม่ ขอส่องไกลๆ ก่อน", "เปลี่ยนบ่อยๆ แล้วฉันไวขึ้นได้", "ถ้าปลอดภัยจะค่อยๆ เข้าใกล้เอง"] },
    heart: { ko: ["말은 없지만 관찰은 꼼꼼하고,", "안전하다고 느끼면 천천히 다가가.", "낯선 변화는 조금 부담스러워."], th: ["ฉันไม่พูดเยอะ แต่ส่องละเอียด", "ถ้ารู้สึกปลอดภัยจะค่อยๆ เข้าใกล้", "ของแปลก/การเปลี่ยนแปลงเยอะๆ ทำให้กังวล"] },
    guide: { ko: ["창가/높은 곳 같은 관찰 포인트를 만들어줘.", "새 환경/새 사람은 ‘천천히’ 적응 시간을.", "예측 가능한 루틴이 안정감을 줘."], th: ["ทำจุดส่อง (หน้าต่าง/ที่สูง) ให้ฉัน", "คนใหม่/ที่ใหม่ ให้เวลาปรับตัวแบบช้าๆ", "รูทีนเดิมๆ ทำให้ใจนิ่ง"] },
...
/** ✅ 고양이 외곽(빈칸) 아이콘 */
function CatOutline({ size = 18, color = "rgba(0,0,0,0.24)" }: { size?: number; color?: string }) {
  return (
